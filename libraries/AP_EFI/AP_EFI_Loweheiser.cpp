...
void AP_EFI_Loweheiser::update()
{
    ...
    if (isnan(pkt.throttle)) {
        ...
        internal_state.throttle_output = 0;
    } else {
        internal_state.throttle_output = pkt.throttle;
    }
    ...
    internal_state.cylinder_status[0].exhaust_gas_temperature = C_TO_KELVIN(pkt.efi_exhaust_gas_temperature);
